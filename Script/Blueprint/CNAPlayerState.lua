...
CNAPlayerState.bIsLobbyReady = false; -- boolean
CNAPlayerState.bIsLobbyTeamLeader = false; -- boolean
...
function CNAPlayerState:SetIsLobbyTeamLeader(IsTeamLeader)
    self.bIsLobbyTeamLeader=IsTeamLeader
    UnrealNetwork.RepLazyProperty(self,"bIsLobbyTeamLeader")
end

function CNAPlayerState:SetIsLobbyReady(bIsReady)

    if UGCGameSystem.IsServer()==false then
        return
    end
